...
if(!isset($_GET['id']) || !is_numeric($_GET['id'])){
    skip('index.php', 'error', '父版块id参数不合法!','css/remind.css');
}
$query="select * from blog_father_module where id={$_GET['id']}";
$result_father=execute($link, $query);
if(mysqli_num_rows($result_father)==0){
    skip('index.php', 'error', '父版块不存在!','css/remind.css');
}
$data_father=mysqli_fetch_assoc($result_father);
$current_item=$data_father['id'];

$query="select * from blog_son_module where father_module_id={$_GET['id']}";
...
$query="select count(*) from blog_content where module_id in({$id_son})";
			...
			<div class="row">
				...
				<div id="mainstay" class="col-md-8">
				    <div class="breadcrumb blog-breadcrumb">
				        ...
				        <a href="index.php" class="home">首页</a> &gt; <span><?php echo $data_father['module_name'];?></span>
				    </div>
					<!-- 文章列表 -->
					<div id="article-list">
                    <?php 
                    $query="select * from blog_content where module_id in({$id_son}) order by id desc {$page['limit']}";
